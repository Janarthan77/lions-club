...
const galleryCategories = [
  { id: 'all', name: 'அனைத்து புகைப்படங்கள்' },
  { id: 'medical', name: 'மருத்துவ முகாம்' },
  { id: 'education', name: 'கல்வி உதவிகள்' },
  { id: 'environment', name: 'பசுமை & மரக்கன்றுகள்' },
...
const galleryItems = [
  ...
  },
  {
    ...
    categoryName: 'பசுமை & மரக்கன்றுகள்',
...
export const Gallery = () => {
  ...
  return (
    <section id="gallery" className="relative py-24 bg-white border-t border-slate-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredItems.map((item) => (
            <div 
              key={item.id}
              className="group relative h-80 rounded-3xl overflow-hidden bg-slate-900 border border-slate-200 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1.5"
            >
              {/* Image */}
              <img 
                src={item.image} 
              ...
              <div className="absolute top-4 left-4 z-10">
                <span className="px-3 py-1 rounded-full text-[10px] font-extrabold text-amber-400 bg-black/40 backdrop-blur-md border border-amber-400/40 uppercase tracking-widest flex items-center gap-1 shadow-sm">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 z-10 flex flex-col gap-1.5">
                <div className="flex items-center gap-2 text-[11px] font-bold text-amber-400/90">
                  <Calendar className="w-3.5 h-3.5" />
                  <span>{item.date}</span>
                </div>
                <h3 className="text-sm font-extrabold text-white font-serif-gold leading-snug line-clamp-2 group-hover:text-amber-300 transition-colors">
                  {item.caption}
                </h3>
              </div>
            </div>
